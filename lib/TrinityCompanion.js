...
require('dotenv').config({ path: '/home/cube/syncstack/opendev-labs/spoon/.env' });
const OllamaEngine = require('./OllamaEngine');
...
class TrinityCompanion {
    ...
    async analyzeIntent(query) {
        ...
        // Check if it's a question first (questions should go to AI, not TANK)
        const isQuestion = /^(how|what|why|when|where|who|can|should|could|would|is|are|do|does|explain|tell|help)\s+/i.test(query);

        if (isQuestion) {
            return { action_required: false, intent: 'chat' };
        }

        // Simple heuristic for "Action Intent" - direct commands only
    ...
    async talk(query) {
        ...
        try {
            ...
            if (process.env.AI_BACKEND === 'ollama') {
                const ollama = new OllamaEngine();
                const prompt = `You are Trinity from The Matrix. You are speaking to ${userName} via a terminal interface (SPOON CLI).
...
- User Projects: ${Array.isArray(stats.projects) ? stats.projects.map(p => p.name).join(', ') : 'none'}

User Query: "${query}"

Respond in character as Trinity. Reference the user's projects if relevant. Keep it under 3 sentences unless asked for a detailed explanation.`;

                const result = await ollama.generateContent(query, prompt);
                finalText = result.response.text();
            } else {
                // Original Google/OpenRouter logic

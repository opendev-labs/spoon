...
const ConstructUI = require('./ConstructUI');
const { extractProjectName } = require('./helpers');

class MatrixAutoPilot {
    constructor(options = {}) {
        const tank = new TankOperator({ ...options, autoPilot: this });
        this.constructUI = new ConstructUI(options.port || 4444);
        this.operators = {
            TANK: tank,
            TRINITY: new TrinityCompanion(tank, this),
            SERAPH: new SeraphProtector(this),
            MORPHEUS: new MorpheusGuide(this),
            ORACLE: new OracleSystem(this),
            PYTHON: new PythonIntegration(this),
            MOUSE: new MouseProgrammer(this),
            SMITH: new SmithAgent(this),
            TRAINER: new MorpheusTrainer(this)
        };
    ...
    broadcast(event, data) {
        if (this.constructUI) {
            this.constructUI.broadcast(event, data);
        }
    }

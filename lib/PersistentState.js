...
class PersistentState {
    ...
    addRelationship(rel) {
        ...
        // Store in LamaDB for Vector-Graph logic
        try {
            const { execSync } = require('child_process');
            const logCmd = `node /home/cube/syncstack/opendev-labs/LamaDB/cli.js data add --vault relationships --data '${JSON.stringify(rel).replace(/'/g, "'\\''")}'`;
    ...
    setAuthChallenge(token) {
        this.state.authChallenge = token;
        this.saveState();
    }
    ...
    addMotif(motif) {
        if (!this.state.motifs) {
            this.state.motifs = [];
        }
        this.state.motifs.push({
            ...motif,
            timestamp: new Date().toISOString()
        });

        // Store in LamaDB as well for cross-project "Golden Thread"
        try {
            const { execSync } = require('child_process');
            const logCmd = `node /home/cube/syncstack/opendev-labs/LamaDB/cli.js data add --vault motifs --data '${JSON.stringify(motif).replace(/'/g, "'\\''")}'`;
            execSync(logCmd, { stdio: 'ignore' });
        } catch (e) { }
